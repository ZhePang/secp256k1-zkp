...
void test_schnorr_adaptor_api(void) {
    ...
    ecount = 0;
    CHECK(secp256k1_schnorr_adaptor_presign(sign, sig, msg, &keypairs[0], t, NULL) == 1);
    CHECK(secp256k1_schnorr_adaptor_extract_t(none, t2, sig, msg, &pk[0]) == 1);
    CHECK(ecount == 0);
    CHECK(secp256k1_schnorr_adaptor_extract_t(sign, t2, sig, msg, &pk[0]) == 1);
    CHECK(ecount == 0);
    CHECK(secp256k1_schnorr_adaptor_extract_t(vrfy, t2, sig, msg, &pk[0]) == 1);
    CHECK(ecount == 0);
    CHECK(secp256k1_schnorr_adaptor_extract_t(vrfy, NULL, sig, msg, &pk[0]) == 0);
    CHECK(ecount == 1);
    CHECK(secp256k1_schnorr_adaptor_extract_t(vrfy, t2, NULL, msg, &pk[0]) == 0);
    CHECK(ecount == 2);
    CHECK(secp256k1_schnorr_adaptor_extract_t(vrfy, t2, sig, NULL, &pk[0]) == 0);
    CHECK(ecount == 3);
    CHECK(secp256k1_schnorr_adaptor_extract_t(vrfy, t2, sig, msg, NULL) == 0);
    CHECK(ecount == 4);
...
void test_schnorr_adaptor_vectors(void) {
    ...
    };
    {
        ...
        const unsigned char sig[65] = {
            0x03, 0x25, 0xD4, 0x9D, 0xDE, 0xD8, 0x17, 0xEF, 
            0xD5, 0xF9, 0xE5, 0xFD, 0x05, 0x57, 0x01, 0xDE, 
            0x28, 0xC6, 0xE6, 0x0F, 0xC2, 0xFC, 0x94, 0x23, 
            0x9F, 0x3C, 0x2C, 0x61, 0xB6, 0x7A, 0xD6, 0xC8, 
            0x2D, 0xDC, 0x21, 0x4B, 0x29, 0xF6, 0x50, 0xBE, 
            0x59, 0x5B, 0x19, 0xD7, 0x9F, 0xE6, 0x10, 0x5E, 
            0x48, 0x15, 0x91, 0x92, 0x60, 0xFD, 0x2C, 0x89, 
            0x8C, 0xF1, 0x1F, 0xD0, 0x33, 0x55, 0x0D, 0xAF, 0x11
        };
        test_schnorr_adaptor_vectors_check_presigning(sk, pk, aux_rand, msg, t, sig);
        test_schnorr_adaptor_vectors_check_extract_t(pk, msg, sig, t, 1);
    };
    {
        ...
        const unsigned char sig[65] = {
            0x03, 0x7C, 0x03, 0x7F, 0xE8, 0xF4, 0xD6, 0x8D, 
    ...
    };
    {
        ...
        const unsigned char sig[65] = {
            0x02, 0xEE, 0x01, 0x73, 0x70, 0x80, 0xBD, 0xCC, 
    ...
    };
    {
        ...
        const unsigned char sig[65] = {
            0x03, 0xFB, 0x4D, 0x87, 0x3E, 0x15, 0x1E, 0xFB, 
            0x14, 0xE1, 0x80, 0xA4, 0x2C, 0x83, 0x47, 0x59, 
            0x59, 0x78, 0xB9, 0x52, 0x91, 0x1A, 0x41, 0x3F, 
            0xA8, 0xFE, 0x33, 0x7C, 0xB0, 0x90, 0x0D, 0x0A, 
            0x47, 0xDD, 0x11, 0xD8, 0xB3, 0x5C, 0xF7, 0xEC, 
            0x18, 0x66, 0x54, 0xA1, 0x01, 0x34, 0xA5, 0xCB, 
            0xEB, 0x43, 0x6A, 0x66, 0x8F, 0x20, 0x04, 0x86, 
            0xC7, 0x58, 0x9E, 0x10, 0x57, 0x27, 0xA8, 0x16, 0xB4
        };
    ...
    };
    {
        ...
        const unsigned char sig[65] = {
            0x02, 0xBC, 0x71, 0xD1, 0xD0, 0xF2, 0x90, 0xAF, 
            0x09, 0x0C, 0x41, 0x6F, 0x30, 0x15, 0x94, 0xDC, 
            0xF5, 0xFC, 0x7C, 0x31, 0xC9, 0xC0, 0x04, 0x6B, 
            0x18, 0xBE, 0x70, 0xC8, 0xD2, 0xC7, 0xC1, 0xDF, 
            0x39, 0x7C, 0x0C, 0xBF, 0x81, 0x18, 0x52, 0x69, 
            0x3B, 0x7C, 0x1B, 0x5B, 0xD7, 0xA0, 0xC4, 0x4A, 
            0x90, 0xEE, 0x00, 0x62, 0x92, 0x7A, 0x64, 0x0B, 
            0x83, 0x83, 0x43, 0xEA, 0xCF, 0xEB, 0xE5, 0xD5, 0x0A
        };
        test_schnorr_adaptor_vectors_check_extract_t(pk, msg, sig, t, 0);
    };
    {
